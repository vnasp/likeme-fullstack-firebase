...
export default function ImagesLike({ image }) {
  ...
  const handleLike = async () => {
    if (!user) {
      setOpenSnackbar(true); // Muestra un Snackbar si el usuario no está logueado
      return;
    }
    ...
    if (alreadyLiked) {
      ...
      if (result.success) {
        ...
        image.likes -= 1; // Actualiza el conteo local de likes para inmediatez en UI
      }
    } else {
  ...
  useEffect(() => {
    // Asegúrate de que image y image.likedBy existen antes de intentar acceder a includes
  ...
  return (
    <>
      ...
      <Box component="span" sx={{ marginRight: 1, color:"white" }}>
        ({image.likes})
      </Box>
      <SnackbarItem openSnackbar={openSnackbar}/>
    </>
  );
}
